...
.loop1
	...
	set STATUSFLAGS_LETTAH_DEX_F, [hl]
	ld a, LETTAH_A
	ld [sGameData + (wFirstLettahSeen - wGameData)], a
	ld hl, sGameData + (wLettahDex - wGameData)
	ld b, NUM_LETTAH
.loop2
	ld [hli], a
	inc a
...
DebugRoomMenu_PokedexClr:
	...
	call OpenSRAM
	ld hl, sPlayerData + (wStatusFlags - wPlayerData)
	res STATUSFLAGS_LETTAH_DEX_F, [hl]
	ld hl, sPlayerData + (wPokedexCaught - wPlayerData)
...
DebugRoomMenu_DecorateAll:
	...
	ld b, SET_FLAG
	ld c, EVENT_DECO_BIG_PLESIOSAUR_DOLL - EVENT_DECO_BED_1 + 1
.loop
	push bc
	push de
	push hl
...
DebugRoomMenu_PokemonGet_Page1Values:
	db 8
	paged_value wDebugRoomMonSpecies,       1,   NUM_POKEMON, CHIVES,      DebugRoom_BoxStructStrings.Pokemon,   DebugRoom_PrintPokemonName, FALSE
