...
_LettahPuzzle:
	...
	xor a
	call ByteFill
	ld hl, LettahPuzzleCursorGFX
	ld de, vTiles0 tile $e0
	ld bc, 4 tiles
	call CopyBytes
	ld hl, LettahPuzzleStartCancelLZ
	ld de, vTiles0 tile $ed
	call Decompress
	call LoadLettahPuzzlePiecesGFX
	hlcoord 0, 0
	...
	lb bc, 12, 12
	ld a, PUZZLE_VOID
	call LettahPuzzle_FillBox
	call InitLettahPuzzlePiecePositions
	call LettahPuzzle_UpdateTilemap
	...
	ld [wHoldingLettahPuzzlePiece], a
	ld [wLettahPuzzleCursorPosition], a
	ld [wLettahPuzzleHeldPiece], a
	ld a, %10010011
	ldh [rLCDC], a
	call WaitBGMap
	ld b, SCGB_LETTAH_PUZZLE
	call GetSGBLayout
	ld a, $e4
	call DmgToCgbBGPals
	ld a, $24
...
.loop
	...
	bit 7, a
	jr nz, .quit
	call LettahPuzzleJumptable
	ld a, [wHoldingLettahPuzzlePiece]
	and a
	jr nz, .holding_piece
...
InitLettahPuzzlePiecePositions:
	ld c, 1
	ld b, 16
.load_loop
	call Random
...
.Function:
	ldh a, [hJoyPressed]
	and START
	jp nz, LettahPuzzle_Quit
	ldh a, [hJoyPressed]
	and A_BUTTON
	jp nz, LettahPuzzle_A
	ld hl, hJoyLast
	ld a, [hl]
	and D_UP
	jr nz, .d_up
...
.d_up
	ld hl, wLettahPuzzleCursorPosition
	ld a, [hl]
	cp 1 puzcoord 0
	ret c
	sub 6
...
LettahPuzzle_A:
	...
	and a
	jr nz, .TryPlacePiece
	call LettahPuzzle_CheckCurrentTileOccupancy
	and a
	jr z, LettahPuzzle_InvalidAction
	ld de, SFX_MEGA_KICK
	call PlaySFX
	ld [hl], 0
	ld [wLettahPuzzleHeldPiece], a
...
.TryPlacePiece:
	call LettahPuzzle_CheckCurrentTileOccupancy
	and a
	jr nz, LettahPuzzle_InvalidAction
	ld de, SFX_PLACE_PUZZLE_PIECE_DOWN
	call PlaySFX
	ld a, [wLettahPuzzleHeldPiece]
	ld [hl], a
	call PlaceLettahPuzzlePieceGFX
	call WaitBGMap
	xor a
	ld [wLettahPuzzleHeldPiece], a
	call RedrawLettahPuzzlePieces
...
LettahPuzzle_InvalidAction:
	ld de, SFX_WRONG
	call PlaySFX
	call WaitSFX
	ret

LettahPuzzle_FillBox:
	ld de, SCREEN_WIDTH
.row
	push bc
	push hl
...
PlaceLettahPuzzlePieceGFX:
	ld a, $2 ; tilemap coords
	call GetLettahPuzzleCoordData
	ld a, [hli]
	ld h, [hl]
	ld l, a
	push hl
...
GetLettahPuzzleCoordData:
	ld e, a
	ld d, 0
	ld hl, LettahPuzzleCoordData
	add hl, de
	ld a, [wLettahPuzzleCursorPosition]
	ld e, a
rept 6
	add hl, de
endr
...
LettahPuzzle_CheckCurrentTileOccupancy:
	ld hl, wPuzzlePieces
	ld a, [wLettahPuzzleCursorPosition]
	ld e, a
	ld d, 0
...
RedrawLettahPuzzlePieces:
	call GetCurrentPuzzlePieceVTileCorner
	ld [wLettahPuzzleCornerTile], a
	xor a
	call GetLettahPuzzleCoordData ; get pixel positions
	ld a, [hli]
	ld b, [hl]
	ld c, a
	ld a, [wLettahPuzzleCornerTile]
	cp $e0
	jr z, .NoPiece
...
.loop
	...
	dec b
	jr nz, .loop
	call LettahPuzzle_AddPuzzlePieceBorders
	ret
...
LettahPuzzle_AddPuzzlePieceBorders:
...
.TileBordersGFX:
INCBIN "gfx/lettah_puzzle/tile_borders.2bpp"

LoadLettahPuzzlePiecesGFX:
	ld a, [wScriptVar]
	maskbits NUM_LETTAH_PUZZLES
	ld e, a
	ld d, 0
	ld hl, .LZPointers
	add hl, de
...
.LZPointers:
; entries correspond to LETTAHPUZZLE_* constants
	dw Shoe_CrabPuzzleLZ
	dw NautilusPuzzleLZ
	dw PteranodonPuzzleLZ
	dw PhoenixPuzzleLZ

LettahPuzzleCursorGFX:
INCBIN "gfx/lettah_puzzle/cursor.2bpp"

LettahPuzzleStartCancelLZ:
INCBIN "gfx/lettah_puzzle/start_cancel.2bpp.lz"

PhoenixPuzzleLZ:
INCBIN "gfx/lettah_puzzle/phoenix.2bpp.lz"

PteranodonPuzzleLZ:
INCBIN "gfx/lettah_puzzle/pteranodon.2bpp.lz"

Shoe_CrabPuzzleLZ:
INCBIN "gfx/lettah_puzzle/shoe_crab.2bpp.lz"

NautilusPuzzleLZ:
INCBIN "gfx/lettah_puzzle/nautilus.2bpp.lz"
