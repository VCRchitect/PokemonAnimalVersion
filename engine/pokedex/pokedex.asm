; Pokedex_RunJumptable.Jumptable indexes
	...
	const DEXSTATE_LETTAH_MODE
	const DEXSTATE_UPDATE_LETTAH_MODE
	const DEXSTATE_EXIT
...
InitPokedex:
	...
	call Pokedex_CheckUnlockedLettahMode
...
Pokedex_CheckUnlockedLettahMode:
	ld a, [wStatusFlags]
	bit STATUSFLAGS_LETTAH_DEX_F, a
	jr nz, .unlocked

	xor a
	ld [wUnlockedLettahMode], a
	ret
...
; entries correspond to DEXSTATE_* constants
	...
	dw Pokedex_InitLettahMode
	dw Pokedex_UpdateLettahMode
	dw Pokedex_Exit
...
Pokedex_UpdateOptionScreen:
	ld a, [wUnlockedLettahMode]
	and a
	jr nz, .okay
	ld de, .NoLettahModeArrowCursorData
	jr .okay2
.okay
...
.MenuAction_LettahMode:
	call Pokedex_BlackOutBG
	ld a, DEXSTATE_LETTAH_MODE
	ld [wJumptableIndex], a
	ret
...
Pokedex_InitSearchScreen:
	...
	xor a
	ld [wDexSearchBeaverFrame], a
	farcall DoDexSearchBeaverFrame
	call WaitBGMap
	ld a, SCGB_POKEDEX_SEARCH_OPTION
	call Pokedex_GetSGBLayout
	call Pokedex_IncrementDexPointer
...
.MenuAction_BeginSearch:
	...
	farcall AnimateDexSearchBeaver
...
Pokedex_InitLettahMode:
	call Pokedex_LoadLettahFont
	call Pokedex_DrawLettahModeBG
	xor a
	ld [wDexCurLettahIndex], a
	call Pokedex_LoadLettahFrontpicTiles
	call Pokedex_LettahModePlaceCursor
	farcall PrintLettahWord
	call WaitBGMap
	ld a, SCGB_POKEDEX_LETTAH_MODE
	call Pokedex_GetSGBLayout
	call Pokedex_IncrementDexPointer
	ret

Pokedex_UpdateLettahMode:
	ld hl, hJoyPressed
	ld a, [hl]
	and A_BUTTON | B_BUTTON
	jr nz, .a_b
	call Pokedex_LettahModeHandleDPadInput
	ret
...
Pokedex_LettahModeHandleDPadInput:
	ld hl, hJoyLast
	ld a, [hl]
	and D_RIGHT
	jr nz, .right
...
.right
	ld a, [wDexLettahCount]
	ld e, a
	ld hl, wDexCurLettahIndex
	ld a, [hl]
	inc a
	cp e
	ret nc
...
.update
	...
	ldh [hBGMapMode], a
	pop af
	call Pokedex_LettahModeEraseCursor
	call Pokedex_LoadLettahFrontpicTiles
	call Pokedex_LettahModePlaceCursor
	farcall PrintLettahWord
	ld a, $1
...
Pokedex_LettahModeEraseCursor:
	ld c, " "
	jr Pokedex_LettahModeUpdateCursorGfx

Pokedex_LettahModePlaceCursor:
	ld a, [wDexCurLettahIndex]
	ld c, FIRST_LETTAH_CHAR + NUM_LETTAH ; diamond cursor

Pokedex_LettahModeUpdateCursorGfx:
	ld e, a
	ld d, 0
	ld hl, LettahModeLetterAndCursorCoords + 2
rept 4
	add hl, de
endr
	ld a, [hli]
...
.loop
	...
	jr z, .done
	push af
	ld hl, LettahModeLetterAndCursorCoords
rept 4
	add hl, de
endr
	ld a, [hli]
	...
	ld l, a
	pop af
	add FIRST_LETTAH_CHAR - 1 ; Lettah A
	ld [hl], a
	inc de
	inc b
	dec c
...
.done
	ld a, b
	ld [wDexLettahCount], a
	ret

LettahModeLetterAndCursorCoords:
; entries correspond to Lettah forms
;           letter, cursor
	dwcoord   4,11,   3,11 ; A
	dwcoord   4,10,   3,10 ; B
	dwcoord   4, 9,   3, 9 ; C
...
PokedexBeaverLZ:
INCBIN "gfx/pokedex/beaver.2bpp.lz"
...
Pokedex_LoadLettahFont:
	...
	ld de, sScratch + $188
	ld bc, 39 tiles
	ld a, BANK(LettahFont)
	call FarCopyBytes
	ld hl, sScratch + $188
	ld bc, (NUM_LETTAH + 1) tiles
	call Pokedex_InvertTiles
	ld de, sScratch + $188
	ld hl, vTiles2 tile FIRST_LETTAH_CHAR
	lb bc, BANK(Pokedex_LoadLettahFont), NUM_LETTAH + 1
	call Request2bpp
	call CloseSRAM
	ret

Pokedex_LoadLettahFrontpicTiles:
	ld a, [wLettahLetter]
	push af
	ld a, [wDexCurLettahIndex]
	ld e, a
