...
.loop
	ld a, PARTYMENUACTION_SWITCH ; Which ANML?
...
BattleMenu_ANML:
	call LoadStandardMenuHeader
BattleMenuANML_ReturnFromStats:
	call ExitMenu
	call LoadStandardMenuHeader
	call ClearBGPalettes
BattleMenuANML_Loop:
...
.PressedB:
	call CheckMobileBattleError
	jr c, .Cancel
	jr BattleMenuANML_Loop

.Stats:
	call Battle_StatsScreen
	call CheckMobileBattleError
	jr c, .Cancel
	jp BattleMenuANML_ReturnFromStats
...
; Force Item1
; Used for Ho-Oh, Ryujin and Panda encounters
	ld a, [wBattleType]
...
; Roaming monsters (Cerberus, Barghest) work differently
...
; If combined with forced shiny battletype, causes an infinite loop
	call CheckLettahLetter
	jr c, .GenerateDVs ; try again

.Carp:
; These filters are untranslated.
; They expect at wCarpLength a 2-byte value in mm,
; but the value is in feet and inches (one byte each).

; The first filter is supposed to make very large Carp even rarer,
; by targeting those 1600 mm (= 5'3") or larger.
; After the conversion to feet, it is unable to target any,
; since the largest possible Carp is 5'3", and $0503 = 1283 mm.
	ld a, [wTempEnemyMonSpecies]
	cp CARP
	jr nz, .Happiness
...
; Barghest and Cerberus will have < 256 hp at level 40
	ld a, [wEnemyMonHP + 1]
	ld [hl], a
	jr .Moves
