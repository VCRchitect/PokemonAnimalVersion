...
GameFreakPresentsInit:
	...
	ld hl, GameFreakSlimeGFX
	ld de, wDecompressScratch
	ld a, BANK(GameFreakSlimeGFX)
	call FarDecompress
...
; If the slime's reached the ground, decrement the jump height and play the sfx
	ld hl, SPRITEANIMSTRUCT_VAR1 ; jump height
	add hl, bc
	ld a, [hl]
	sub 48
	ld [hl], a
	ld de, SFX_SLIME_BOUNCE
	call PlaySFX
	ret
...
; Fade slime's palettes while it's transforming
	srl a
	srl a
	ld e, a
	ld d, 0
	ld hl, GameFreakSlimePaletteFade
	add hl, de
	add hl, de
	ldh a, [rSVBK]
	push af
...
GameFreakSlimePaletteFade:
INCLUDE "gfx/splash/slime_fade.pal"
