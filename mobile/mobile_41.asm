...
StubbedTrainerRankings_CarpLength:
	ret
	ld a, BANK(sTrainerRankingLongestCarp)
	call OpenSRAM
	ld de, wCarpLength
	ld hl, sTrainerRankingLongestCarp

	; Is this Carp the longest measured?
	ld a, [de]
	cp [hl]
...
.checkShortest
	; First, check if the record for shortest Carp is 0.
	; This seems unnecessary, because the value is initialized to 100.0 cm.
	ld hl, sTrainerRankingShortestCarp
	ld a, [hli]
	or [hl]
	dec hl
	jr z, .newRecordShortest

	; Now check if this Carp is the shortest
	ld a, [de]
	cp [hl]
	jr z, .isLowByteLower
...
; Initializes Trainer Rankings data for a new save file in JP Crystal.
	...
	; Initialize the shortest Carp to 100.0 cm
	ld hl, sTrainerRankingShortestCarp
	ld a, $3
	ld [hli], a
	ld [hl], $e8
