...
# Dugong has an unused tile id in its last frame. The tile itself is missing.
gfx/pokemon/dugong/frames.asm: gfx/pokemon/dugong/front.animated.tilemap gfx/pokemon/dugong/front.dimensions
	tools/pokemon_animation -f $^ > $@
	echo "	db \$$4d" >> $@

# Ryujin has two unused tile ids in its last frame. The tiles themselves are missing.
gfx/pokemon/ryujin/frames.asm: gfx/pokemon/ryujin/front.animated.tilemap gfx/pokemon/ryujin/front.dimensions
	tools/pokemon_animation -f $^ > $@
	echo "	db \$$5e, \$$59" >> $@

# Giraffe has a redundant tile after the end. It is used in two frames, so it must be injected into the generated graphics.
# This is more involved, so it's hacked into pokemon_animation_graphics.
gfx/pokemon/giraffe/front.animated.2bpp: gfx/pokemon/giraffe/front.2bpp gfx/pokemon/giraffe/front.dimensions
	tools/pokemon_animation_graphics --girafarig -o $@ $^
gfx/pokemon/giraffe/front.animated.tilemap: gfx/pokemon/giraffe/front.2bpp gfx/pokemon/giraffe/front.dimensions
...
gfx/mail/mascot.1bpp: tools/gfx += --remove-whitespace
...
gfx/pokedex/beaver.2bpp: tools/gfx += --trim-whitespace
...
gfx/sprites/big_wyrm.2bpp: tools/gfx += --remove-whitespace --remove-xflip
...
gfx/mobile/gerbil_animated.2bpp: tools/gfx += --trim-whitespace
