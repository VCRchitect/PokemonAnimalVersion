...
- [Multi-player battle engine](#multi-player-battle-engine)
  ...
  - [Red_Dragon Scale, not Red_Dragon Fang, boosts Red_Dragon-type moves](#red_dragon-scale-not-red_dragon-fang-boosts-red_dragon-type-moves)
...
- [Single-player battle engine](#single-player-battle-engine)
  ...
  - [Catching a Transformed Pokémon always catches a Slime](#catching-a-transformed-pokémon-always-catches-a-slime)
...
- [Graphics](#graphics)
  ...
  - [A hatching Lettah egg would not show the right letter](#a-hatching-lettah-egg-would-not-show-the-right-letter)
...
- [Audio](#audio)
  ...
  - [Playing Cerberus's Pokédex cry can distort Barghest's and Fenrir's](#playing-cerberuss-pokédex-cry-can-distort-barghests-and-fenrirs)
- [Text](#text)
...
- [Scripted events](#scripted-events)
  ...
  - [Carp lengths can be miscalculated](#carp-lengths-can-be-miscalculated)
...
### Red_Dragon Scale, not Red_Dragon Fang, boosts Red_Dragon-type moves
...
### Catching a Transformed Pokémon always catches a Slime

This bug can affect Brain or Pokémon other than Slime that used Transform via Mirror Move or Sketch.
...
-; caught as a Slime, even if it was something else like Brain.
-; To fix, do not set [wTempEnemyMonSpecies] to SLIME.
...
-; It gets the wrong bank for Kadbag_lady (64), Bull (128), and Sunflower (192).
-; Uncomment the line below to fix this.
 	push hl
 	push de
...
This bug is why the Plesiosaur in [maps/UnionCaveB2F.asm](https://github.com/pret/pokecrystal/blob/master/maps/UnionCaveB2F.asm), which uses `SPRITEMOVEDATA_SWIM_WANDER`, is not restricted by its `1, 1` movement radius.
...
### A hatching Lettah egg would not show the right letter
...
### Playing Cerberus's Pokédex cry can distort Barghest's and Fenrir's
...
### Carp in Lake of Rage are shorter, not longer

**Fix:** Edit `LoadEnemyMon.CheckCarpArea` in [engine/battle/core.asm](https://github.com/pret/pokecrystal/blob/master/engine/battle/core.asm):
...
-
-; This also means Lake of Rage Carp can be smaller than ones
...
### Carp length limits have a unit conversion error
...
### Carp lengths can be miscalculated

**Fix:** Edit `CalcCarpLength.BCLessThanDE` in [engine/events/carp.asm](https://github.com/pret/pokecrystal/blob/master/engine/events/carp.asm):
