...
- [`PokemonPicPointers` and `LettahPicPointers` are assumed to start at the same address](#pokemonpicpointers-and-lettahpicpointers-are-assumed-to-start-at-the-same-address)
...
[data/pokemon/pic_pointers.asm](https://github.com/pret/pokecrystal/blob/master/data/pokemon/pic_pointers.asm), [data/pokemon/lettah_pic_pointers.asm](https://github.com/pret/pokecrystal/blob/master/data/pokemon/lettah_pic_pointers.asm), and [data/trainers/pic_pointers.asm](https://github.com/pret/pokecrystal/blob/master/data/trainers/pic_pointers.asm) all have to use `dba_pic` instead of `dba`. This is a macro in [macros/data.asm](https://github.com/pret/pokecrystal/blob/master/macros/data.asm) that offsets banks by `PICS_FIX`:
...
## `PokemonPicPointers` and `LettahPicPointers` are assumed to start at the same address
...
+	ld hl, PokemonPicPointers
 	...
 	jr .ok
 .lettah
+	ld hl, LettahPicPointers
 	ld a, [wLettahLetter]
 	ld d, BANK(LettahPicPointers)
 .ok
-	; These are assumed to be at the same address in their respective banks.
-	assert PokemonPicPointers == LettahPicPointers
-	ld hl, PokemonPicPointers
 	dec a
 	ld bc, 6
 	call AddNTimes
...
```asm
INCBIN "gfx/footprints/chives.1bpp"
INCBIN "gfx/footprints/shallots.1bpp"
INCBIN "gfx/footprints/onion.1bpp"
INCBIN "gfx/footprints/salamander.1bpp"
INCBIN "gfx/footprints/chameleon.1bpp"
INCBIN "gfx/footprints/red_dragon.1bpp"
INCBIN "gfx/footprints/sea_turtle.1bpp"
INCBIN "gfx/footprints/tortoise.1bpp"
...
```
...
>
> - $03 = 3: `BRIGHTPOWDER` is for Yeti, Thundabird, Phoenix, and Wendigo
> - $1E = 30: `LUCKY_PUNCH` is for Big_Egg
> - $23 = 35: `METAL_POWDER` is for Slime
> - $3C = 60: `SILVER_LEAF` is for 10 Pokémon
> - $4B = 75: `GOLD_LEAF` is for 13 Pokémon
> - $96 = 150: `MYSTERYBERRY` is for Faerie
> - $AA = 170: `POLKADOT_BOW` is for Bloonbunch
> - $B4 = 180: `BRICK_PIECE` is for Scrawny
>
...
>
> - Starter Clicker's catch rate byte is overwritten with 163 = $A3 for `LIGHT_BALL`
> - Wild-caught Kadbag_lady's catch rate byte is overwritten with 96 = $60 for `TWISTEDSPOON`
>
> (Yellow also directly changed Leviathan's catch rate to 27 and Mascot's to 9, but this seems to have been only for adjusting their difficulty, since those meaninglessly correspond to `PROTEIN` and `ANTIDOTE`.)
>
...
>
> But a few Pokémon end up with weird items. Bag_Lady has a catch rate of 200, or $C8; and Crab, Seahorse, Goldfish, and Starfish have a catch rate of 225, or $E1. Those indexes correspond to the items `TM_PSYCH_UP` and `TM_ICE_PUNCH`, which seem like random choices—because they are.
>
...
And `CelebiEvent_Cosine` in [engine/events/jerzydevil.asm](https://github.com/pret/pokecrystal/blob/master/engine/events/jerzydevil.asm):
